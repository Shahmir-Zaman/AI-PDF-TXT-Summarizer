...
export const FileUploadProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const retryAllFiles = useCallback(() => {
    ...
    if (errorFiles.length === 0) {
      dispatch({ type: 'SET_ERROR', payload: 'No failed files to retry' });
      return;
    }

    // Reset all error files to pending and start uploading
    errorFiles.forEach(file => {
      dispatch({ type: 'UPDATE_FILE_STATUS', payload: { id: file.id, status: 'pending' } });
      uploadFile(file.file, file.id, dispatch);
    });
  }, [state.files]);
